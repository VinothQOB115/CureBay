...
//		ReportManager.logInfoAPI("actual response code is - " + actualResponseCode);
		ReportManager.logInfo("<summary><font color=\"green\"><b>Actual Response Code is - </b></font></summary>" + actualResponseCode);
//      ReportManager.logInfoAPI("expected response code is - " + ExpectedStatusCode);
        ReportManager.logInfo("<summary><font color=\"green\"><b>Expected response code is - </b></font></summary>" + ExpectedStatusCode);
...
//		ReportManager.logInfoAPI("actual content type is - " + ActualContentType);
		ReportManager.logInfo("<summary><font color=\"green\"><b>Actual content type is - </b></font></summary>" + ActualContentType);
//		ReportManager.logInfoAPI("expected content type is - " + ExpectedContentType);
		ReportManager.logInfo("<summary><font color=\"green\"><b>Expected content type is - </b></font></summary>" + ExpectedContentType);
...
//			
				JsonAssertions.assertThatJson(inPath(response, table.get(row).get("JPath")))
						.when(Option.IGNORING_ARRAY_ORDER).isEqualTo(table.get(row).get("Value"));
				ReportManager.logInfo(table.get(row).get("JPath") + " - " + table.get(row).get("Value"));
	...
	public void verifyNodeIsPresent(DataTable dataTable) {
		...
		for (int row = 0; row < table.size(); row++) {
			...
			ReportManager.logInfo(table.get(row).get("JPath"));
		}
	}
...
//		ReportManager.logInfoAPI("Response time is - " + apiTestContext.getApiTestBase().response.time());
		ReportManager.logInfo("<summary><font color=\"green\"><b>Response time is - </b></font></summary>" + apiTestContext.getApiTestBase().response.time());
	}
